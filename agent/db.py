...
def today_str() -> str:
    """오늘 날짜 (YYYY-MM-DD), 사용자 타임존 기준. 백업 파일명 등에 씁니다."""
    return _today().isoformat()


def create_backup_copy() -> str:
    """DB의 안전한 스냅샷 파일을 만들고 그 경로를 반환합니다.

    파일을 그냥 복사(shutil.copy)하면 마침 쓰기 중인 페이지가 섞여 손상된
    백업이 될 수 있습니다. sqlite3의 온라인 백업 API(Connection.backup)는
    다른 연결이 동시에 읽고 쓰는 중이어도 그 순간의 일관된 상태를 통째로
    복사해줍니다. 호출자가 다 쓰고 나면 이 경로의 파일을 지워야 합니다.
    """
    fd, path = tempfile.mkstemp(suffix=".db", prefix="maneul-backup-")
    os.close(fd)
