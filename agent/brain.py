...
def build_system_prompt() -> str:
    ...
    memos = db.list_memos()
    memos_block = "\n".join(f"- {m['text']} ({m['created_at'][:10]})" for m in memos) if memos else "- (아직 없음)"
...
- **메모**: 사용자가 `/memo`로 남긴 메모를 항상 참고합니다. `/memos`로 전체 목록을 볼 수 있어요.
...
**"저는 대화 기록을 검색 못 해요"는 사실이 아닙니다.** 최근 대화와 저장된 메모는 항상 보고 있습니다.
위 목록대로 정확히 답하세요. 모르면 모른다고 하되, 할 수 있는 걸 못 한다고 하지 마세요.
